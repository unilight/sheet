...
if [ "${stage}" -le -1 ] && [ "${stop_stage}" -ge -1 ]; then
    ...
    ../nisqa/local/data_download.sh "${nisqa_db_root}"
fi
...
if [ "${stage}" -le 0 ] && [ "${stop_stage}" -ge 0 ]; then
    ...
    for test_set in LIVETALK FOR P501; do
        ../nisqa/local/data_prep.py \
            ...
            --resample --target-sampling-rate "${target_sampling_rate}" --target-wavdir "${nisqa_db_root}/NISQA_TEST_${test_set}/deg_${target_sampling_rate}"
    done
fi

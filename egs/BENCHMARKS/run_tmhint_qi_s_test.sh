...
tmhintqis_db_root=/data/group1/z44476r/Corpora/tmhint-qi-s    # change this to your dataset folder
# tmhintqis_db_root=../tmhint-qi-s/downloads
datadir="../tmhint-qi-s/data"
...
if [ ${stage} -le 0 ] && [ ${stop_stage} -ge 0 ]; then
    ...
    ../tmhint-qi-s/local/data_prep.py \
        --original-path "${tmhintqis_db_root}" \
        --wavdir "${tmhintqis_db_root}/wavs" \
        --answer_path "../tmhint-qi-s/answers/answer.csv" \
        --out "${datadir}/tmhint_qi_s_test.csv" \
        --resample --target-sampling-rate "${target_sampling_rate}" --target-wavdir "${tmhintqis_db_root}/wavs_${target_sampling_rate}"
fi

...
# test_sets="dev test"
test_sets="dev test LIVETALK FOR P501"
...
if [ ${stage} -le 0 ] && [ ${stop_stage} -ge 0 ]; then
    ...
    for test_set in ${nisqa_test_sets}; do
        ../nisqa/local/data_prep.py \
            --original-path "${nisqa_db_root}/NISQA_TEST_${test_set}/NISQA_TEST_${test_set}_file.csv" --wavdir "${nisqa_db_root}/NISQA_TEST_${test_set}/deg" --out "data/${test_set}.csv" 
    done
fi
